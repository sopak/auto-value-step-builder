 ...
 */
package cz.jcode.auto.value.step.builder.example.kotlin.generic

import com.google.common.collect.ImmutableList
import cz.jcode.auto.value.step.builder.example.generic.Address
import cz.jcode.auto.value.step.builder.example.generic.NoBuilderAddress
import cz.jcode.auto.value.step.builder.example.generic.Person
...
class GenericTest {
    ...
    fun step() {
        ...
        val person = Person.step<String, Address<String>, NoBuilderAddress<String>>()
                .id(PERSON_ID)
    ...
    fun lazyStep() {
        ...
        val personBuilder = Person.lazyStep<String, Address<String>, NoBuilderAddress<String>>()
                .id { PERSON_ID }
                .names { ImmutableList.of(PERSON_NAME, PERSON_SURNAME) }
                .phones { ImmutableList.of(PERSON_PHONE) }
                .homeAddress(
                        Address.lazyStep<String>()
                                .title { ADDRESS_TITLE }
                                .street { ADDRESS_STREET }
                                .city { ADDRESS_CITY }
                                .postcode { ADDRESS_POSTCODE }
    ...
    fun lazyStepSuppliedPersonId() {
        ...
        val limit = 1000
        IntStream.range(0, limit).parallel().forEach { personBuilder.build() }

        assertEquals(idSupplier.get(), limit + 2)
    }

    private fun assertPerson(person: Person<String, Address<String>, NoBuilderAddress<String>>) {
    ...
    companion object {
        ...
        val PERSON_WORK_ADDRESS: NoBuilderAddress<String>? = null
